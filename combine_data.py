...
def combine_data():
    ...
    df = pd.concat(all_data, ignore_index=True).drop(['username', 'ip_addr_decrypted', 'user_agent_decrypted', 'platform', 'conn_country', 'episode_name', 'episode_show_name', 'spotify_episode_uri','offline_timestamp','offline','incognito_mode', 'shuffle','reason_start','reason_end','skipped'], axis=1) # might do things with columns like conn_country later, but for now just drop them
    print("Data combined")
